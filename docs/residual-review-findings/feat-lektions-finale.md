...
| ~~P4~~ | ~~Konsistenz~~ | **Erledigt durch Merge.** Der Plan `docs/superpowers/plans/2026-07-31-lektions-finale.md` verwies an vier Stellen auf `LessonEmojis`, das zum Zeitpunkt der Umsetzung nur auf `feat/pfad-screen-kindgerecht` existierte. Dieser Branch wurde inzwischen in `main` gemergt — `content/LessonEmojis.kt` existiert jetzt, und die vier Plan-Verweise sind keine toten Verweise mehr. Die während der Umsetzung vorgenommenen Ausweich-Verweise auf `WordFrameSizing`/`SyllableFrameSizing` (in `PRODUCT_PRINCIPLES.md` Abschnitt 12, der Design-Spec und `ContentValidator.kt`) wurden zurück auf `LessonEmojis` gestellt — das ist die inhaltlich treffendere Analogie (gleiche Domäne: deterministische Emoji-Auswahl fürs Lektions-Vokabular, Dedupe auf dem Glyph statt der Atom-ID), nicht nur die zufällig verfügbare. |
